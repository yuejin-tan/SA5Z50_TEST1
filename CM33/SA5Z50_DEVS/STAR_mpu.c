...
void set_region_base(uint32_t base,uint8_t share_att,uint8_t ap_att, uint8_t xn){
		MPU->RBAR = (base & MPU_CTRL_RBAR_BASE_MASK) |  //from addr 0
							  (((uint32_t)share_att << MPU_CTRL_RBAR_SH_SHIFT) & MPU_CTRL_RBAR_SH_MASK) |
                (((uint32_t)ap_att << MPU_CTRL_RBAR_AP_SHIFT) & MPU_CTRL_RBAR_AP_MASK) | //r&w
...
void set_mair(uint8_t idx,uint8_t att){
	if(idx<=3){
		MPU->MAIR0 &= ~(0xFFU << (idx * 8)); //crate mask of index 0
		MPU->MAIR0 |=  ((uint32_t)att << (idx * 8));  //set value of index 0.device-ngnrne
	}
	else if(idx>3&&idx<=7){
		MPU->MAIR1 &= ~(0xFFU << ((idx - 4) * 8)); //crate mask
  	MPU->MAIR1 |=  ((uint32_t)att << ((idx - 4) * 8));  //set value
	}	
}
