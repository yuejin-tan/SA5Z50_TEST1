...
void lv_port_disp_init(void)
{
	 ...
	 *----------------------------*/
	  ...
	  * */
	  ...
	  // static lv_color_t draw_buf_1[LV_HOR_RES_MAX * 48];						  /*A buffer for 16 rows*/
	  // lv_disp_buf_init(&draw_buf_dsc_1, draw_buf_1, NULL, LV_HOR_RES_MAX * 48); /*Initialize the display buffer*/
	...
	lv_color_t* color_p)
{
	...
	uint32_t pixCnt = (width + 1) * (height + 1);
	...
	LL_DMA_ConfigAddresses(DMA2,
		LL_DMA_STREAM_0,
		(uint32_t)color_p, (uint32_t) & (LCD->LCD_RAM),
		LL_DMA_DIRECTION_MEMORY_TO_MEMORY);
	LL_DMA_SetDataLength(DMA2, LL_DMA_STREAM_0, pixCnt);
	// LL_DMA_SetStreamPriorityLevel(DMA2, LL_DMA_STREAM_0, LL_DMA_PRIORITY_LOW);
	LL_DMA_EnableIT_TC(DMA2, LL_DMA_STREAM_0);

	disp_drv_save = disp_drv;
